...
public final class AutoCloser implements AutoCloseable {
    ...
    /**
     * Stores the given throwable and rethrows it <i>as is</i> if it is a {@code Exception}, {@code RuntimeException} or
     * {@code Error}. In the unlikely possibility that it is a throwable instance, it will be rethrown wrapped in a
    ...
    public void close() throws Exception {
        ...
        while (!stack.isEmpty()) {
            final AutoCloseable resource = stack.pop();
            try {
                resource.close();
